...
export default function ClientChatbot({ agente, color }: { agente: any; color: string }) {
  ...
  const [messages, setMessages] = useState([{ role: "bot", text: `Hola! Soy ${agente.nombre}. Para ayudarte mejor, me regalas tu nombre?` }]);
  ...
  async function sendMsg() {
    ...
    if (step === "nombre") {
      ...
      await delay(1200);
      setLoading(false);
      setMessages(prev => [...prev, { role: "bot", text: `Mucho gusto ${userText}! Me regalas tu correo?` }]);
      return;
    }
    ...
    if (step === "celular") {
      const nuevoLead = { ...lead, celular: userText };
      setLead(nuevoLead);
      setStep("chat");
      setLoading(true);
      await delay(1500);
      setLoading(false);
      setMessages(prev => [...prev, { role: "bot", text: `Listo ${lead.nombre}! En que te puedo ayudar?` }]);
      return;
    }
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "bot", text: "Ups, algo salio mal. Intenta de nuevo." }]);
    }
    setLoading(false);
  }
  ...
  return (
    <>
      ...
      {open && (
        <div style={{ position: "fixed", bottom: 84, right: 24, width: 340, maxHeight: 500, background: "#fff", borderRadius: 20, boxShadow: "0 8px 40px rgba(0,0,0,0.18)", zIndex: 9999, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {loading && (
              <div style={{ background: "#f3f4f6", borderRadius: "12px 12px 12px 0", padding: "10px 14px", fontSize: 13, color: "#888", display: "flex", gap: 4, alignItems: "center" }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#aaa", animation: "pulse 1s infinite" }} />
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#aaa", animation: "pulse 1s infinite 0.2s" }} />
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#aaa", animation: "pulse 1s infinite 0.4s" }} />
          ...
          <div style={{ display: "flex", gap: 8, padding: 12, borderTop: "1px solid #f0f0f0" }}>
            <input
              ...
              placeholder={step === "nombre" ? "Tu nombre..." : step === "correo" ? "Tu correo..." : step === "celular" ? "Tu celular..." : "Escribe aqui..."}
              style={{ flex: 1, border: "1px solid #e5e7eb", borderRadius: 10, padding: "8px 12px", fontSize: 13, outline: "none" }}
            />
            <button onClick={sendMsg} disabled={loading} style={{ background: color, color: "#fff", border: "none", borderRadius: 10, padding: "8px 14px", cursor: "pointer", fontWeight: 700, fontSize: 13, opacity: loading ? 0.7 : 1 }}>
