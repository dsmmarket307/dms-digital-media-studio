  ...
  return (
    <div style={{ padding: "2rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
        {(() => {
          const promedioMostrado = producto.calif_promedio ? parseFloat(producto.calif_promedio) : promedio;
          const cantidadMostrada = producto.calif_cantidad !== undefined && producto.calif_cantidad !== "" ? parseInt(producto.calif_cantidad) : totalResenas;
          const textoMostrado = producto.calif_texto || `Resena${cantidadMostrada !== 1 ? "s" : ""} verificada${cantidadMostrada !== 1 ? "s" : ""}`;
          return (
            <>
              <span style={{ fontSize: "1rem" }}>{"⭐".repeat(Math.round(promedioMostrado)) || "⭐⭐⭐⭐⭐"}</span>
              <span style={{ fontSize: "0.9rem", fontWeight: 700, color: "#111" }}>{promedioMostrado > 0 ? promedioMostrado.toFixed(1) : "5.0"}</span>
              <span style={{ fontSize: "0.85rem", color: "#666" }}>({cantidadMostrada} {textoMostrado})</span>
            </>
          );
        })()}
