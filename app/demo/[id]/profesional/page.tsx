...
export default async function DemoProfesional({ params }: Props) {
  ...
  const rubrosConsulta = ["Abogados","Contaduria","Consultoria","Arquitectura","Construccion"];
  const waMensaje = rubrosCita.includes(rubro) ? `Hola, me interesa agendar una valoracion en ${nombreNeg}. Podrian darme mas informacion sobre disponibilidad y servicios?` : rubrosPedido.includes(rubro) ? `Hola, quiero hacer un pedido en ${nombreNeg}. Me pueden dar mas informacion?` : rubrosConsulta.includes(rubro) ? `Hola, quiero agendar una consulta en ${nombreNeg}. Podrian darme mas informacion sobre sus servicios?` : `Hola, quiero mas informacion sobre ${nombreNeg}.`;
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .hero-body h1{font-size:2.25rem}section{padding:4rem 1.5rem}.nosotros-grid,.contact-grid,.compare-cards,.footer-grid{grid-template-columns:1fr}.galeria-grid{grid-template-columns:1fr 1fr}.stats{grid-template-columns:1fr}}
  ...
  return (
    <>
      ...
      {c?.barraAnuncio?.activo && c?.barraAnuncio?.items?.length > 0 && (
        <div style={{ background: c.barraAnuncio.colorFondo || "#111111", overflow: "hidden", padding: "8px 0" }}>
          <div style={{ display: "flex", width: "max-content", animation: "marquee 20s linear infinite" }}>
            {[...c.barraAnuncio.items, ...c.barraAnuncio.items].map((txt: string, i: number) => (
              <span key={i} style={{ color: c.barraAnuncio.colorTexto || "#f5c542", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", padding: "0 20px", display: "flex", alignItems: "center", gap: 20 }}>
                {txt}
      ...
      {site.template_version === "v2" && c?.marquesina?.activo && c?.marquesina?.texto && (
        <div className="offer-marquee">
          <div className="offer-marquee-track">
            {Array(2).fill(c.marquesina.texto).map((t: string, i: number) => (
      ...
      <nav>
        <div className="brand">
          {logo && <img src={logo} alt="logo" style={{ height: 56, maxWidth: 200, objectFit: "contain" }} />}{!logo && <h1>{c?.footer?.nombre_empresa ?? site.project_name}</h1>}
        </div>
            ...
            {site.template_version === "v2" && Array.from(new Set(c.productos.map((p: any) => p.categoria).filter(Boolean))).length > 1 && (
              <div className="cat-pills" style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", marginBottom: "2rem" }}>
                {Array.from(new Set(c.productos.map((p: any) => p.categoria).filter(Boolean))).map((cat: any, i: number) => (
                  <a key={i} href={`/demo/${id}/categoria/${encodeURIComponent(cat)}`} style={{ padding: "0.5rem 1.25rem", borderRadius: 999, border: `1px solid ${pr}`, color: pr, fontSize: "0.85rem", fontWeight: 600, textDecoration: "none", whiteSpace: "nowrap" }}>
                    {cat}
                  </a>
                ))}
              </div>
                ...
                <a key={`prod-${i}`} href={`/demo/${id}/producto/${(c.productos as any[]).indexOf(p)}?from=profesional`} className="srv-card" style={{ textDecoration: "none", color: "inherit", padding: 0, overflow: "hidden", position: "relative" }}>
                  {p.imagenes?.length > 0 ? (
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ position: "absolute", top: 10, left: 10, background: "#e11d48", color: "#fff", fontSize: "0.75rem", fontWeight: 800, padding: "0.25rem 0.6rem", borderRadius: 4 }}>
                          -{pct}% OFF
                        </div>
                      );
                    })()}
        ...
        <section id="nosotros">
          <div className="wrap">
            <div className="nosotros-grid">
              <div>{(ci.nosotros || imagenes[1]) && <ParallaxImage src={ci.nosotros || imagenes[1]} alt="nosotros" className="nosotros-img" />}</div>
      ...
      {c?.contacto?.whatsapp_flotante && c?.contacto?.whatsapp && (
        <WhatsappWidget nombre={nombreNeg} mensaje={waMensaje} link={`https://wa.me/${c.contacto.whatsapp.replace(/\D/g,"")}?text=${encodeURIComponent(waMensaje)}`} color={pr} rebote={c?.contacto?.whatsapp_rebote} />
      )}
