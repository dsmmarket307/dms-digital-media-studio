  ...
  return (
    <>
      ...
      {c?.contacto?.whatsapp_flotante && c?.contacto?.whatsapp && (
        <a href={`https://wa.me/${c.contacto.whatsapp.replace(/\D/g,"")}?text=${encodeURIComponent(waMensaje)}`} target="_blank" rel="noopener noreferrer" className={`wa-flotante ${c?.contacto?.whatsapp_rebote ? "wa-bounce-anim" : ""}`} style={{ position: "fixed", bottom: 24, right: 96, zIndex: 999, background: "#25D366", width: 56, height: 56, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(0,0,0,0.3)" }}>
