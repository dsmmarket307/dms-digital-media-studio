...
function UpgradeBadge({ plan }: { plan: string }) {
  return (
    <div style={{ background:"linear-gradient(135deg,#7c3aed,#4f46e5)", borderRadius:12, padding:"16px 20px", textAlign:"center", margin:"12px 0" }}>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" style={{ marginBottom:8, display:"block", margin:"0 auto 8px" }}><path d="M12 2l3.09 6.26L22 9.27l-5 4.87L18.18 21 12 17.77 5.82 21 7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
      ...
      <Link href="/dashboard/client/suscripcion" style={{ background:"#fff", color:"#7c3aed", padding:"8px 20px", borderRadius:8, fontSize:12, fontWeight:700, textDecoration:"none", display:"inline-block" }}>
        Ver planes
      </Link>
    </div>
  );
    ...
    return (
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display:"flex", gap:6 }}>
          <button onClick={() => triggerImg(target)} style={{ flex:1, padding:"8px", borderRadius:8, border:`1px dashed ${pr}`, background:`${pr}08`, color:pr, fontSize:12, fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
            {uploadingImg === target ? "Subiendo..." : "Cambiar imagen"}
          </button>
          {pexelsQuery && (
            <button onClick={() => refreshPexels(target, pexelsQuery)} style={{ padding:"8px 10px", borderRadius:8, border:`1px dashed ${pr}`, background:`${pr}08`, color:pr, fontSize:12, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }} title="Nueva imagen automatica">
  ...
  if (!miSitio || editando) {
    return (
      <div style={{ minHeight:"100vh", background:"#f8f9fa", display:"flex" }}>
        ...
        <aside style={{ width:240, background:"#fff", borderRight:"1px solid #e5e7eb", minHeight:"100vh", position:"sticky", top:0, display:"flex", flexDirection:"column", flexShrink:0 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
              ...
              <div style={{ background:"#fff", borderRadius:14, border:"1px solid #e5e7eb", padding:20 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                  ...
                  <div>
                    <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#888", marginBottom:6, textTransform:"uppercase" as const }}>Tema {!canChangeTipo && <span style={{ fontSize:10, background:"#f3f4f6", color:"#aaa", padding:"1px 6px", borderRadius:4, marginLeft:4 }}>PRO</span>}</label>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      {TEMAS.map(t => (
                        <button key={t} onClick={() => canChangeTipo && setForm({ ...form, theme: t })} disabled={!canChangeTipo} style={{ padding:"10px", borderRadius:8, fontSize:12, fontWeight:500, border:"1px solid", borderColor: form.theme===t ? pr : "#e5e7eb", background: form.theme===t ? `${pr}10` : "#fff", color: form.theme===t ? pr : "#888", cursor: canChangeTipo ? "pointer" : "not-allowed", opacity: canChangeTipo ? 1 : 0.5 }}>
                          {t}
                        </button>
                      ))}
                    </div>
    ...
    <div style={{ height:"100vh", background:"#f0f0f0", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #e5e7eb", padding:"10px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexShrink:0, zIndex:50 }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <button onClick={() => handleSave(true)} style={{ background:pr, color:"#fff", border:"none", borderRadius:8, padding:"6px 14px", fontSize:12, fontWeight:700, cursor:"pointer" }}>
            Publicar
          </button>
        </div>
      </div>
