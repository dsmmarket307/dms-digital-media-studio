            ...
            {tickets.length === 0 ? (
              ...
              tickets.map(t => (
                <div key={t.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                  <div className="flex items-start justify-between mb-3 flex-wrap gap-2">
                    <div>
                      ...
                      <div className="flex gap-3 mt-1 flex-wrap">
                        ...
                        <p className="text-xs text-gray-400">{new Date(t.created_at).toLocaleDateString("es-CO", { day:"2-digit", month:"long", year:"numeric", hour:"2-digit", minute:"2-digit" })}</p>
                      </div>
                    </div>
                  ...
                  <p className="text-sm text-gray-600 bg-gray-50 rounded-xl p-4 leading-relaxed mb-4">{t.mensaje}</p>
                  {t.respuesta && (
                    <div className="border-l-4 border-purple-400 pl-4 mb-4 bg-purple-50 py-3 rounded-r-xl">
                      <p className="text-xs font-semibold text-purple-600 mb-1">Tu respuesta:</p>
                      <p className="text-sm text-gray-700 leading-relaxed">{t.respuesta}</p>
                    </div>
                  )}
                  {t.estado !== "cerrado" && (
              ...
              leads.map(l => (
                <div key={l.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                  ...
                  <div className="flex gap-2 mt-4 flex-wrap">
                    <a href={`mailto:${l.email}`} className="text-xs bg-purple-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-purple-700 transition-colors">
                      Enviar email
                    </a>
                    {l.telefono && (
                      <a href={`https://wa.me/57${l.telefono.replace(/\D/g,"")}`} target="_blank" className="text-xs bg-green-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-600 transition-colors">
                        WhatsApp
                      </a>
                    )}
                  </div>
      ...
      {respondiendo && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full shadow-xl">
            ...
            <div className="bg-gray-50 rounded-xl p-3 mb-4 text-sm text-gray-600 leading-relaxed">{respondiendo.mensaje}</div>
