...
export default function PageBuilderEditor() {
  ...
  function updateProductoOferta(prodIndex: number, ofertaIndex: number, field: string, value: any) {
    setContent((prev: any) => {
      const next = JSON.parse(JSON.stringify(prev));
      if (!next.productos[prodIndex].ofertas) {
        next.productos[prodIndex].ofertas = [
      ...
      next.productos[prodIndex].ofertas[ofertaIndex][field] = value;
      return next;
    });
  }
  ...
  return (
    <div style={{ height: "100vh", background: "#f0f0f0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        {/* PANEL IZQUIERDO */}
        <div style={{ width: view === "desktop" ? 180 : 220, background: "#fff", borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", overflow: "auto", flexShrink: 0 }}>
        ...
        <div style={{ width: view === "desktop" ? 400 : "auto", flex: view === "desktop" ? "0 0 auto" : 1, background: "#fff", borderLeft: "1px solid #e5e7eb", overflow: "auto", flexShrink: 0 }}>
          ...
          <div style={{ padding: "14px" }}>
              ...
              {content?.productos?.map((p: any, i: number) => (
                <div key={i} style={{ marginBottom: 14, background: "#f8f9fa", borderRadius: 10, padding: "12px" }}>
                  ...
                  <div style={{ marginBottom: 8 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <input type="color" value={p.boton_color ?? primaryColor} onChange={e => updateArray("productos", i, "boton_color", e.target.value)} style={{ width: 30, height: 30, borderRadius: 6, border: "1px solid #e5e7eb", cursor: "pointer" }} />
                      <span style={{ fontSize: 10, color: "#555" }}>{p.boton_color ?? primaryColor}</span>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, background: "#f8f9fa", borderRadius: 10, padding: "8px 10px" }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#111" }}>Mostrar boton Agregar al carrito</span>
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    {[
                      { index: 0, label: "1 unidad" },
                      { index: 1, label: "2 unidades" },
                      { index: 2, label: "3 unidades" },
                      { index: 3, label: "4 unidades" },
                    ].map(({ index: oi, label: olabel }) => {
                      const oferta = (p.ofertas ?? [{ cantidad: 1, descuento: 0, oculta: false }, { cantidad: 2, descuento: 15, oculta: false }, { cantidad: 3, descuento: 25, oculta: false }, { cantidad: 4, descuento: 35, oculta: false }])[oi];
                      return (
                        <div key={oi} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, background: "#fff", borderRadius: 8, padding: "8px 10px", border: "1px solid #e5e7eb" }}>
                          <span style={{ fontSize: 11, color: "#555", flex: 1 }}>{olabel}</span>
                          <input type="number" min={0} max={100} value={oferta?.descuento ?? 0} onChange={e => updateProductoOferta(i, oi, "descuento", parseInt(e.target.value))} style={{ width: 50, border: "1px solid #e5e7eb", borderRadius: 6, padding: "4px 6px", fontSize: 11, textAlign: "center" }} />
                          <span style={{ fontSize: 10, color: "#888" }}>%</span>
                          <button onClick={() => updateProductoOferta(i, oi, "oculta", !(oferta?.oculta ?? false))} style={{ padding: "3px 8px", borderRadius: 6, border: "1px solid #e5e7eb", background: (oferta?.oculta ?? false) ? "#f3f4f6" : `${pr}15`, color: (oferta?.oculta ?? false) ? "#aaa" : pr, fontSize: 10, fontWeight: 700, cursor: "pointer" }}>
                            {(oferta?.oculta ?? false) ? "OFF" : "ON"}
                          </button>
                        </div>
                      );
                    })}
            ...
            {selectedSection === "confianza" && (<>
              ...
              {(content?.confianza ?? [
                { icono: "ðŸšš", texto: "Envio GRATIS" },
                { icono: "ðŸ’µ", texto: "Pago Contra Entrega" },
                { icono: "ðŸ”’", texto: "Compra 100% Segura" },
                { icono: "â­", texto: "Calidad Garantizada" },
              ]).map((item: any, ci: number) => (
                <div key={ci} style={{ display: "flex", gap: 6, marginBottom: 8, alignItems: "center" }}>
                  <input value={item.icono} onChange={(e) => { setContent((prev: any) => { const next = JSON.parse(JSON.stringify(prev)); if (!next.confianza) next.confianza = [{ icono: "ðŸšš", texto: "Envio GRATIS" }, { icono: "ðŸ’µ", texto: "Pago Contra Entrega" }, { icono: "ðŸ”’", texto: "Compra 100% Segura" }, { icono: "â­", texto: "Calidad Garantizada" }]; next.confianza[ci].icono = e.target.value; return next; }); }} style={{ width: 44, padding: "6px 4px", border: "1px solid #e5e7eb", borderRadius: 6, fontSize: 14, textAlign: "center", outline: "none" }} />
                  <input value={item.texto} onChange={(e) => { setContent((prev: any) => { const next = JSON.parse(JSON.stringify(prev)); if (!next.confianza) next.confianza = [{ icono: "ðŸšš", texto: "Envio GRATIS" }, { icono: "ðŸ’µ", texto: "Pago Contra Entrega" }, { icono: "ðŸ”’", texto: "Compra 100% Segura" }, { icono: "â­", texto: "Calidad Garantizada" }]; next.confianza[ci].texto = e.target.value; return next; }); }} placeholder="Texto" style={{ flex: 1, padding: "6px 8px", border: "1px solid #e5e7eb", borderRadius: 6, fontSize: 11, outline: "none" }} />
                  <button onClick={() => { setContent((prev: any) => { const next = JSON.parse(JSON.stringify(prev)); if (!next.confianza) next.confianza = [{ icono: "ðŸšš", texto: "Envio GRATIS" }, { icono: "ðŸ’µ", texto: "Pago Contra Entrega" }, { icono: "ðŸ”’", texto: "Compra 100% Segura" }, { icono: "â­", texto: "Calidad Garantizada" }]; next.confianza.splice(ci, 1); return next; }); }} style={{ background: "#fef2f2", color: "#ef4444", border: "none", borderRadius: 6, padding: "6px 8px", fontSize: 11, cursor: "pointer" }}>x</button>
                </div>
              ))}
              <button onClick={() => { setContent((prev: any) => { const next = JSON.parse(JSON.stringify(prev)); if (!next.confianza) next.confianza = [{ icono: "ðŸšš", texto: "Envio GRATIS" }, { icono: "ðŸ’µ", texto: "Pago Contra Entrega" }, { icono: "ðŸ”’", texto: "Compra 100% Segura" }, { icono: "â­", texto: "Calidad Garantizada" }]; next.confianza.push({ icono: "âœ…", texto: "Nuevo mensaje" }); return next; }); }} style={{ width: "100%", padding: "8px", borderRadius: 8, border: `1px dashed ${pr}`, background: `${pr}08`, color: pr, fontSize: 12, fontWeight: 600, cursor: "pointer", marginTop: 4 }}>+ Agregar mensaje</button>
            </>)}
            ...
            {selectedSection === "cookies" && (<>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, background: "#f8f9fa", borderRadius: 10, padding: "10px 12px" }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: "#111" }}>Mostrar aviso de cookies</span>
                <button onClick={() => setContent((prev: any) => { const next = JSON.parse(JSON.stringify(prev)); if (!next.cookies) next.cookies = {}; next.cookies.activo = next.cookies.activo === false ? true : false; return next; })} style={{ width: 40, height: 22, borderRadius: 999, border: "none", cursor: "pointer", background: (content?.cookies?.activo ?? true) ? pr : "#d1d5db", position: "relative", transition: "background 0.2s" }}>
                  <span style={{ position: "absolute", top: 2, left: (content?.cookies?.activo ?? true) ? 20 : 2, width: 18, height: 18, borderRadius: "50%", background: "#fff", transition: "left 0.2s", boxShadow: "0 1px 3px rgba(0,0,0,0.3)" }} />
                </button>
              </div>
                    ...
                    {(p.items || []).map((it: any, ii: number) => (
                      ...
                      >
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, color: "#999", display: "flex", alignItems: "center", gap: 4 }}>â ¿ Item {ii + 1}</span>
